...
CREATE TABLE user_accounts (
    ...
    user_password VARCHAR(100) NOT NULL CHECK (LENGTH(user_password) >= 8),
...
CREATE TABLE enthusiasts (
    ...
    user_email VARCHAR(100) REFERENCES user_accounts(user_email) ON DELETE CASCADE
);
...
CREATE TABLE sport_rules (
    ...
    updated_at DATE,
    sport_type_id INT REFERENCES sport_types(sport_type_id) ON DELETE CASCADE
);

CREATE TABLE illustrations (
    illustration_id SERIAL PRIMARY KEY,
    image_url VARCHAR(255) CHECK (image_url ~* '^https?://'),
    image_type VARCHAR(50),
    description TEXT,
    rule_id INT REFERENCES sport_rules(rule_id) ON DELETE CASCADE
);
...
CREATE TABLE addresses (
    ...
    entrance_number INT,
    floor INT,
    enthusiast_id INT REFERENCES enthusiasts(enthusiast_id) ON DELETE CASCADE
);
...
CREATE TABLE water_orders (
    ...
    water_type_id INT REFERENCES water_types(water_type_id) ON DELETE SET NULL,
    volume_id INT REFERENCES water_volumes(volume_id) ON DELETE SET NULL,
    supplier_id INT REFERENCES suppliers(supplier_id) ON DELETE SET NULL
);
...
CREATE TABLE chat_rooms (
    ...
    event_id INT REFERENCES sport_events(event_id) ON DELETE CASCADE
);
...
CREATE TABLE messages (
    ...
    enthusiast_id INT REFERENCES enthusiasts(enthusiast_id) ON DELETE CASCADE,
    chat_id INT REFERENCES chat_rooms(chat_id) ON DELETE CASCADE
);
